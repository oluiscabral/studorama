...
export default function StudySessionComponent() {
  ...
  const [apiSettings] = useLocalStorage('studorama-api-settings', { openaiApiKey: '' });
  ...
  const loadNextQuestion = async (session: StudySession) => {
    ...
    try {
      const questionData = await generateQuestion(session.subject, apiSettings.openaiApiKey);
