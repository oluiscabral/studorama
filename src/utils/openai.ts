export async function generateQuestion(subject: string, apiKey: string): Promise<any> {
  if (!apiKey) {
...
export async function evaluateAnswer(question: string, userAnswer: string, correctAnswer: string, apiKey: string): Promise<string> {
  if (!apiKey) {
    throw new Error('OpenAI API key is required');
  }
